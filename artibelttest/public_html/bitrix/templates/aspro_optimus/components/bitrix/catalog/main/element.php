...
if ($arElement['PREVIEW_PICTURE'] || $arElement['DETAIL_PICTURE']) {
	$rsFile = CFile::GetByID($arElement['PREVIEW_PICTURE'] ? $arElement['PREVIEW_PICTURE'] : $arElement['DETAIL_PICTURE']);
	if ($arFile = $rsFile->Fetch()) {
		$imageWidth = $arFile['WIDTH'];
		$imageHeight = $arFile['HEIGHT'];
		$imageType = $arFile['CONTENT_TYPE'];
	}
}
?>
<?COptimus::AddMeta(
	array(
		...
		'og:image:type' => $imageType ?? false,
		'og:image:width' => $imageWidth ?? false,
		'og:image:height' => $imageHeight ?? false,
	)
);
$APPLICATION->SetPageProperty('description', $arElement['PREVIEW_TEXT']);
?>
